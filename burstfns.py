...
def sub_ts(frbname, dm, nchan, ffac, avgfac, tbasems, fmhz0, tpeakms):
	...
	if((os.path.exists(itsfile)) and (os.path.exists(stksdsfile)) and (os.path.exists(compfile))):
		...
		for si in range(0,nsub):
			ax0		=	fig.add_subplot(nsub+1,1,si+1)
			#for cs in comedges[2:-1]:
				#plt.axvline(cs,c='grey',ls='--', lw=0.8)
			plt.axhline(c='k', ls='--', lw=0.5)
			plt.plot(reltms, isubts[si],c=colarr[si%nsub], ls='-',lw=0.6,label="%d MHz"%(np.nanmedian(fmhzarr[subchs[si][0]:subchs[si][1]])))
			plt.xlim((tbasems[0], tbasems[1]))		
		...
		plt.savefig("{}{}_sub_{}_bands_{}_{}_avg_{}_{}.png".format(plotdir,frbname,nsub,dm,nchan,ffac,avgfac))
		plt.show()
		del(stksds)
		del(its)
	else:
...
def estimate_rm(frbname, dm, nchan, ffac, avgfac, fmhz0, lwms, rwms, phirange, dphi, startchan, endchan, tpeakms):
	...
	if(os.path.exists(stksdsfile) and os.path.exists(noisefile)):
		...
		(fdfpeak, peakrm, epeakrm, peakpa)	=	rm_synth_clean(fmhzarr[goodc0]/1.0e3, iqu, eiqu, phirange, dphi, 1.0)
		res_rmsynth[0]	=	peakrm[1]
		res_rmsynth[1]	=	epeakrm[1]
		res_rmsynth[2]	=	peakpa[1]
		...
		#fr_rmenst(fmhzarr/1.0e3, fmhz0/1.0e3, iquv, eiquv)
	...
	print("\nResults from RMsynth (RM synthesis) \n")
	print("RM = %.2f +/- %.2f rad/m2   PolAng0 = %.2f +/- %.2f deg\n"%(res_rmsynth[0], res_rmsynth[1], res_rmsynth[2], res_rmsynth[3]))
	
	print("\nResults from RMnest (RM fitting) \n")
	print("RM = %.2f +/- %.2f rad/m2   PolAng0 = %.2f +/- %.2f deg\n"%(res_rmnest[0], res_rmnest[1], res_rmnest[2], res_rmnest[3]))
